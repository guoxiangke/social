<div class="block block-interest block-add-people">
	<h2 class="block-title">可能感兴趣的人</h2>
	<ul class="block-content unstyled">
		<li class="stbody">
			<a href="#" class="stimg stimgL"><img src="http://www.w3cplus.com/sites/default/files/styles/user_picture/public/pictures/picture-1-1347153884.jpg" alt=""></a>
			<div class="sttext">
				<h3 class="title"><a href="">大漠</a></h3>
				<div class="btn-wrap"><a href="#"><i class="addicon">+</i>关注</a></div>
			</div>
		</li>
		<li class="stbody">
			<a href="#" class="stimg stimgL"><img src="http://www.w3cplus.com/sites/default/files/styles/user_picture/public/pictures/picture-1-1347153884.jpg" alt=""></a>
			<div class="sttext">
				<h3 class="title"><a href="">大漠</a></h3>
				<div class="btn-wrap"><a href="#"><i class="cancelicon">-</i>取消关注</a></div>
			</div>
		</li>
		<li class="stbody">
			<a href="#" class="stimg stimgL"><img src="http://www.w3cplus.com/sites/default/files/styles/user_picture/public/pictures/picture-1-1347153884.jpg" alt=""></a>
			<div class="sttext">
				<h3 class="title"><a href="">大漠</a></h3>
				<div class="btn-wrap"><a href="#"><i class="addicon">+</i>关注</a></div>
			</div>
		</li>
		<li class="stbody">
			<a href="#" class="stimg stimgL"><img src="http://www.w3cplus.com/sites/default/files/styles/user_picture/public/pictures/picture-1-1347153884.jpg" alt=""></a>
			<div class="sttext">
				<h3 class="title"><a href="">大漠</a></h3>
				<div class="btn-wrap"><a href="#"><i class="addicon">+</i>关注</a></div>
			</div>
		</li>
		<li class="stbody">
			<a href="#" class="stimg stimgL"><img src="http://www.w3cplus.com/sites/default/files/styles/user_picture/public/pictures/picture-1-1347153884.jpg" alt=""></a>
			<div class="sttext">
				<h3 class="title"><a href="">大漠</a></h3>
				<div class="btn-wrap"><a href="#"><i class="addicon">+</i>关注</a></div>
			</div>
		</li>		
	</ul>
</div>