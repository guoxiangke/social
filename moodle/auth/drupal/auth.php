...
class auth_plugin_drupal extends auth_plugin_base {
  ...
  function loginpage_hook() {
    ...
    $host = !empty($CFG->d7_cookie_domain) ? $CFG->d7_cookie_domain : $_SERVER['HTTP_HOST'];
    $cookie = 'SESS' . substr(hash('sha256', $host), 0, 32);
    $d7_sid = $_COOKIE[$cookie];
    if (empty($d7_sid)) {
      return TRUE;
    }
    // Check whether Drupal session is authenticated
    $d7_user = $DB->get_record_sql("SELECT u.* FROM d7_users u
      LEFT JOIN d7_sessions s on (s.uid = u.uid)
      WHERE s.sid = '$d7_sid' AND s.uid <> 0");
    if ($d7_user === FALSE) {
      return TRUE;
    }
    // Fetch or create the corresponding Moodle user.
    $user = $this->create_update_user($d7_user);
  ...
  function create_update_user($d7_user) {
    global $CFG;
    global $DB;

    $user = $DB->get_record('user', array('idnumber' => $d7_user->uid));
    $name = explode(' ', $d7_user->name, 2);
    $user->firstname = !empty($name[0]) ? $name[0] : ' ';
    $user->lastname = !empty($name[1]) ? $name[1] : ' ';
    $user->idnumber = $d7_user->uid;
    $user->email = $d7_user->mail;
    $user->auth = 'drupal';
    $user->username = $d7_user->name;
    $user->modified = time();
  ...
  function sync_users($do_updates = FALSE) {
    ...
    if (!empty($this->config->removeuser)) {
      ...
      $remove_users = $DB->get_records_sql("SELECT u.id, u.username, u.email, u.auth " .
        ...
        "(SELECT name COLLATE utf8_unicode_ci FROM d7_users users)"
      );
    ...
    if (count($users_to_sync)) {
      foreach ($users_to_sync as $username) {
        ...
        echo "\n";
        $user = $DB->get_record_sql("SELECT users.* FROM d7_users users " .
  ...
  function logoutpage_hook() {
    ...
    if (!empty($d7_sid)) {
      ...
      //$DB->execute_sql("DELETE FROM d7_sessions sessions WHERE sid = '$d7_sid'");
    }
  }

  function user_exists($username) {
    return $DB->record_exists_sql("SELECT uid FROM d7_users users WHERE name = '$username'");
  }
  ...
  function get_userlist() {
    ...
    $result = array();
    // fetch userlist
    $user_names = $DB->get_recordset_sql("SELECT name AS username FROM d7_users users WHERE uid > 0");
